...
exports.formatTaskForLLM = formatTaskForLLM;
 ...
 */
function analyzeTask(userInput, activeFilePath) {
...
function detectTarget(input, activeFilePath) {
...
function extractTargetDescription(input, _type) {
...
function extractConstraints(input) {
    ...
    if (lower.includes("without") || lower.includes("don't") || lower.includes("shouldn't")) {
        ...
        if (negativeMatch)
            constraints.push(`Avoid: ${negativeMatch[1].trim()}`);
    }
    if (lower.includes("must") || lower.includes("should") || lower.includes("need to")) {
        ...
        if (positiveMatch)
            constraints.push(`Must: ${positiveMatch[1].trim()}`);
    }
    // Framework constraints
    if (lower.includes("angular"))
        constraints.push("Follow Angular conventions");
    if (lower.includes("react"))
        constraints.push("Follow React patterns");
    if (lower.includes("typescript"))
        constraints.push("Use strict TypeScript");
    if (lower.includes("accessibility") || lower.includes("a11y"))
        constraints.push("Ensure accessibility");
    if (lower.includes("responsive"))
        constraints.push("Make responsive");
    if (lower.includes("backward compatible"))
        constraints.push("Maintain backward compatibility");
    return constraints;
}
function detectIssueType(input) {
...
function extractIssueDetails(input) {
...
function extractExpectedOutcome(input) {
...
function assessComplexity(input, files, classes) {
    ...
    if (hasMigration || (hasMultipleTargets && hasIntegration))
        return "complex";
    if (wordCount > 30 || hasMultipleTargets)
        return "moderate";
    return "simple";
}
function assessScope(input, files) {
...
function normalizeInput(input, action, target) {
...
function formatTaskForLLM(task) {
    ...
    if (task.issueType) {
        ...
        if (task.currentBehavior)
            parts.push(`- **Current:** ${task.currentBehavior}`);
    ...
    if (task.mentionedClasses.length || task.mentionedMethods.length || task.mentionedFiles.length) {
        parts.push(`\n### Referenced Code`);
        if (task.mentionedFiles.length)
            parts.push(`- Files: ${task.mentionedFiles.join(", ")}`);
        if (task.mentionedClasses.length)
            parts.push(`- Classes: ${task.mentionedClasses.join(", ")}`);
        if (task.mentionedMethods.length)
            parts.push(`- Methods: ${task.mentionedMethods.join(", ")}`);
    }
    // Constraints
