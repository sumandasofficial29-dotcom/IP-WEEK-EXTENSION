 ...
 */
function analyzeDependencyImpact(targetFilePath, projectRoot) {
    ...
    const summary = generateImpactSummary(dependentFiles, importedModules, exposedExports, impactLevel, warnings);
    return {
        dependentFiles,
        importedModules,
        exposedExports,
...
function findDependentFiles(projectRoot, targetName, _targetRelativePath, targetExports) {
    const dependents = [];
...
function searchDirForImports(dir, patterns, targetName, targetExports, results) {
    try {
...
function mapExportsToUsages(exports, dependents) {
...
function calculateImpactLevel(dependents, exports) {
    const dependentCount = dependents.length;
    const totalUsages = exports.reduce((sum, e) => sum + e.usageCount, 0);
    if (dependentCount === 0)
        return "low";
    if (dependentCount >= 10 || totalUsages >= 50)
        return "critical";
    if (dependentCount >= 5 || totalUsages >= 20)
        return "high";
    if (dependentCount >= 2 || totalUsages >= 5)
        return "medium";
    return "low";
}
function generateWarnings(dependents, exports, imports) {
    const warnings = [];
...
function generateImpactSummary(dependents, imports, exports, impactLevel, warnings) {
    const lines = [];
