...
export interface PromptQualityResult {
  overallScore: number;
  likelihood: string;  // "High", "Medium", "Low" chance of accurate response
 ...
 */
export function analyzePromptQuality(input: AnalysisInput): PromptQualityResult {
  ...
  if (taskClarity.issue) criticalIssues.push(taskClarity.issue);
  if (taskClarity.suggestion) improvements.push(taskClarity.suggestion);

  // 2. Target Specificity (20% weight)
  const targetSpec = analyzeTargetSpecificity(input);
  dimensions.push(targetSpec);
  if (targetSpec.issue) criticalIssues.push(targetSpec.issue);
  if (targetSpec.suggestion) improvements.push(targetSpec.suggestion);

  // 3. Context Relevance (25% weight)
  const contextRelevance = analyzeContextRelevance(input);
  dimensions.push(contextRelevance);
  if (contextRelevance.issue) criticalIssues.push(contextRelevance.issue);
  if (contextRelevance.suggestion) improvements.push(contextRelevance.suggestion);

  // 4. Actionability (20% weight)
  const actionability = analyzeActionability(input.userTask, input);
  dimensions.push(actionability);
  if (actionability.issue) criticalIssues.push(actionability.issue);
  if (actionability.suggestion) improvements.push(actionability.suggestion);

  // 5. Technical Context (10% weight)
  const techContext = analyzeTechnicalContext(input);
  dimensions.push(techContext);
  if (techContext.suggestion) improvements.push(techContext.suggestion);
 ...
 */
function analyzeTaskClarity(task: string): QualityDimension {
 ...
 */
function analyzeTargetSpecificity(input: AnalysisInput): QualityDimension {
 ...
 */
function analyzeContextRelevance(input: AnalysisInput): QualityDimension {
 ...
 */
function analyzeActionability(task: string, input: AnalysisInput): QualityDimension {
 ...
 */
function analyzeTechnicalContext(input: AnalysisInput): QualityDimension {
  let score = 30; // Base score
  let suggestion: string | undefined;
 ...
 */
export function explainQuality(result: PromptQualityResult): string {
