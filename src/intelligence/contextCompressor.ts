...
export class ContextCompressor {
  ...
  private formatProjectInfo(scan: RepoScanResult, tech: TechStackResult): string {
    const lines = [
      "# Project Overview",
      "",
  ...
  private formatTechStack(tech: TechStackResult): string {
    ...
    if (build.bundler || build.transpiler) {
      ...
      if (build.transpiler) lines.push(`- **Transpiler:** ${build.transpiler}`);
      lines.push("");
    }
    ...
    if (styling.framework || styling.preprocessor || styling.methodology) {
      lines.push("## Styling");
      if (styling.framework) lines.push(`- **UI Framework:** ${styling.framework}`);
      if (styling.preprocessor) lines.push(`- **Preprocessor:** ${styling.preprocessor}`);
      if (styling.methodology) lines.push(`- **Methodology:** ${styling.methodology}`);
      lines.push("");
    }
    ...
    if (testing.framework || testing.e2e) {
      lines.push("## Testing");
      if (testing.framework) lines.push(`- **Unit Testing:** ${testing.framework}`);
      if (testing.e2e) lines.push(`- **E2E Testing:** ${testing.e2e}`);
      if (testing.coverage) lines.push(`- **Coverage:** ${testing.coverage}`);
      lines.push("");
    }
  ...
  private formatArchitecture(tech: TechStackResult): string {
    ...
    if (api.client || api.style) {
      ...
      if (api.style) lines.push(`- **Style:** ${api.style}`);
      if (api.client) lines.push(`- **Client:** ${api.client}`);
      lines.push("");
    }
  ...
  private formatDependencies(tech: TechStackResult): string {
  ...
  private formatStructure(scan: RepoScanResult): string {
