...
export class EnhancedTechStackDetector {
  ...
  private sourceFileExtensionCounts: Record<string, number> = {};  // Only src/ files
  private totalFiles: number = 0;
  private totalSourceFiles: number = 0;
  ...
  private detectPrimaryLanguage(): string {
    ...
    for (const [ext, count] of Object.entries(countsToUse)) {
      ...
      if (lang && !lang.includes("Header") && !lang.includes("Config")) {
        ...
        if (EnhancedTechStackDetector.CONFIG_EXTENSIONS.has(ext)) continue;
   ...
   */
  private verifyLanguageHasFiles(language: string): boolean {
