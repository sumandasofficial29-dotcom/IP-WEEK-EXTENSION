...
export interface CodeSection {
  ...
  endLine: number;
  content: string;
  reason: string;  // Why this section is relevant
}

/**
 * Extracts targeted context based on the analyzed task
 */
export function extractTargetedContext(
  task: AnalyzedTask,
  projectRoot: string,
  ...
  if (activeFilePath && activeFileContent) {
    context.primaryFile = analyzeFile(
      activeFilePath,
      activeFileContent,
      projectRoot,
      task
  ...
  if (context.primaryFile) {
    ...
    context.usedBy = findUsages(
      path.basename(context.primaryFile.path, ".ts"),
      projectRoot
    );
  }
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const term of searchTerms) {
      if (line.includes(term)) {
        ...
        const overlapping = sections.find(
          s => s.startLine <= endLine && s.endLine >= startLine
        );
...
function deduplicateSections(sections: CodeSection[]): CodeSection[] {
  // Sort by start line
  sections.sort((a, b) => a.startLine - b.startLine);
  
  const merged: CodeSection[] = [];
 ...
 */
function extractFileOverview(lines: string[]): string {
...
function findFileByName(rootPath: string, fileName: string): string | undefined {
  const searchDirs = ["src", "lib", "app"];
  
  for (const dir of searchDirs) {
    const result = searchInDir(path.join(rootPath, dir), fileName);
    if (result) return path.relative(rootPath, result);
  }
  
  return undefined;
}

function searchInDir(dir: string, fileName: string): string | undefined {
  try {
...
function findFileContaining(rootPath: string, searchTerm: string): string | undefined {
  const searchDirs = ["src", "lib", "app"];
...
function searchContentInDir(dir: string, searchTerm: string): string | undefined {
  try {
    const entries = fs.readdirSync(dir, { withFileTypes: true });
...
function extractRelatedClasses(
  task: AnalyzedTask,
  availableClasses: ClassInfo[]
): ClassContext[] {
  const related: ClassContext[] = [];
...
function findUsages(symbolName: string, projectRoot: string): string[] {
  const usages: string[] = [];
...
function findUsagesInDir(dir: string, symbolName: string, results: string[]): void {
  try {
    const entries = fs.readdirSync(dir, { withFileTypes: true });
...
function generateContextSummary(context: TargetedContext, _task: AnalyzedTask): string {
  const parts: string[] = [];
 ...
 */
export function formatTargetedContext(context: TargetedContext): string {
  const parts: string[] = [];
