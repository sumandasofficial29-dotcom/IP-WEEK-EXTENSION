...
const configCache = new Map<string, { data: ProjectConfig; timestamp: number }>();
const testingCache = new Map<string, { data: TestingConfig; timestamp: number }>();
 ...
 */
export function readTestingConfig(rootPath: string): TestingConfig {
  // Check cache first
  ...
  if (fs.existsSync(packageJsonPath)) {
    try {
      ...
      if (scripts.test) config.commands.push(`npm test`);
      if (scripts["test:unit"]) config.commands.push(`npm run test:unit`);
      if (scripts["test:e2e"]) config.commands.push(`npm run test:e2e`);
      if (scripts["test:integration"]) config.commands.push(`npm run test:integration`);
      
      // Detect test framework
      const deps = { ...pkg.dependencies, ...pkg.devDependencies };
      if (deps.jest) config.framework = "Jest";
      else if (deps.mocha) config.framework = "Mocha";
      else if (deps.vitest) config.framework = "Vitest";
      else if (deps.cypress) config.framework = "Cypress";
      else if (deps["@playwright/test"]) config.framework = "Playwright";
    } catch { /* skip */ }
  }
  ...
  for (const mp of makefilePaths) {
    if (fs.existsSync(mp)) {
      try {
        ...
        if (content.match(/^test\s*:/m)) config.commands.push("make test");
        if (content.match(/^check\s*:/m)) config.commands.push("make check");
        if (content.match(/^unittest\s*:/m)) config.commands.push("make unittest");
        if (!config.framework) config.framework = "Make";
        break;
      } catch { /* skip */ }
    }
  }
 ...
 */
function extractRelevantReadmeSections(content: string): string {
 ...
 */
function extractRelevantContributingSections(content: string): string {
  const lines = content.split("\n");
 ...
 */
function extractRelevantCopilotInstructions(content: string): string {
  const lines = content.split("\n");
  ...
  if (extractedGuidelines.length === 0) {
    ...
    for (const line of lines) {
      if (keyPatterns.some(p => p.test(line.trim()))) {
        ...
        if (extractedGuidelines.length >= 20) break; // Limit to 20 key bullets
      }
    }
  }
 ...
 */
function extractTestInstructions(content: string): string {
  const lines = content.split("\n");
  const relevantLines: string[] = [];
  let lineCount = 0;
  ...
  for (const line of lines) {
    ...
    if (isHeading) {
      ...
      lineCount = 0;
      if (inRelevant) relevantLines.push(line);
 ...
 */
function detectBuildSystem(rootPath: string): BuildSystemConfig {
  ...
  for (const mf of makefilePaths) {
    ...
    if (fs.existsSync(mfPath)) {
      ...
      try {
        ...
        if (content.match(/^test\s*:/m)) config.testCommands.push("make test");
        if (content.match(/^check\s*:/m)) config.testCommands.push("make check");
      } catch { /* skip */ }
  ...
  if (fs.existsSync(packageJsonPath)) {
    ...
    try {
      const pkg = JSON.parse(fs.readFileSync(packageJsonPath, "utf-8"));
      const scripts = pkg.scripts || {};
      if (scripts.build) config.buildCommands.push("npm run build");
      if (scripts.compile) config.buildCommands.push("npm run compile");
      if (scripts.test) config.testCommands.push("npm test");
    } catch { /* skip */ }
    return config;
  }
 ...
 */
function findComponentDocs(rootPath: string): ComponentDoc[] {
 ...
 */
function extractVersioningInfo(content: string): string {
  const lines = content.split("\n").slice(0, 30);
  const relevantLines: string[] = [];
