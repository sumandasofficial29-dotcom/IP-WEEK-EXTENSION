...
exports.generateFocusedPrompt = generateFocusedPrompt;
 ...
 */
function generateFocusedPrompt(input) {
    const projectConfig = (0, projectConfigReader_1.readProjectConfig)(input.rootPath);
    const testingConfig = (0, projectConfigReader_1.readTestingConfig)(input.rootPath);
    const sections = [];
 ...
 */
function buildDependencySection(impact) {
    const parts = [];
 ...
 */
function buildRoleSection(input) {
    const personas = {
 ...
 */
function buildTestSection(input, testConfig) {
    const lines = [];
    // Detect Robot Framework: by test framework name, active .robot file, or detected frameworks list
    const isRobotFramework = testConfig.framework?.toLowerCase().includes("robot") ||
 ...
 */
function buildRequirementsSection(input, testConfig) {
    const requirements = [];
    ...
    switch (input.task.action) {
        ...
        case "create": {
            ...
            if (opts?.includeExplanation !== false) {
                requirements.push(`${n++}. **Explain approach** - Why this design was chosen`);
            }
            requirements.push(`${n++}. **Usage example** - How to use the new code`);
            if (opts?.includeTests) {
                requirements.push(`${n++}. **Unit tests** - See Unit Tests section above`);
            }
            break;
        }
        case "refactor":
...
function generateCustomPrompt(input, options) {
    const projectConfig = (0, projectConfigReader_1.readProjectConfig)(input.rootPath);
    const testingConfig = (0, projectConfigReader_1.readTestingConfig)(input.rootPath);
    const parts = [];
    // Role (expert persona)
 ...
 */
function getDefaultOptions(task) {
    const defaults = {
...
//# sourceMappingURL=focusedPromptGenerator.js.map