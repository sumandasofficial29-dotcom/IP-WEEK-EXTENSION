...
function buildSmartContext(input) {
    const { classes, relevantFiles, activeFilePath, activeFileContent, mode } = input;
    const relevantClasses = classes.filter(c => relevantFiles.includes(c.filePath));
    const activeClass = relevantClasses.find(c => c.filePath === activeFilePath);
...
Methods:
${activeClass.methods
            .map(m => `- ${m.name}(${m.parameters.join(", ")}) : ${m.returnType}`)
            .join("\n")}
`;
    }
...
//# sourceMappingURL=smartContextBuilder.js.map